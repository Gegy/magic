...
public record DrawGlyphStrokeC2SEvent(@Nullable GlyphNode node) {
    ...
    public static DrawGlyphStrokeC2SEvent start(@NotNull final GlyphNode node) {
    ...
    private void encode(final FriendlyByteBuf buf) {
        GlyphNode.PACKET_CODEC.encode(node, buf);
    }

    private static DrawGlyphStrokeC2SEvent decode(final FriendlyByteBuf buf) {
        final GlyphNode node = GlyphNode.PACKET_CODEC.decode(buf);
        return new DrawGlyphStrokeC2SEvent(node);
    }
}
